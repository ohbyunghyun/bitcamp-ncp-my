...
public class BoardHandler {
  ...
  private void printBoard(StreamTool streamTool) throws Exception {
    ...
    if (b == null) {
      streamTool.println("해당 번호의 게시글 없습니다.");
      return;
    }
  ...
  private void modifyBoard(StreamTool streamTool) throws Exception {
    ...
    if (str.equalsIgnoreCase("Y")) {
      ...
      streamTool.println("변경했습니다.").send();
    } else {
      streamTool.println("변경 취소했습니다.").send();
    }
  ...
  public void menu(StreamTool streamTool) throws Exception {
    streamTool.printf("[%s]\n", this.title)
    .println("1. 등록")
    .println("2. 목록")
    .println("3. 조회")
